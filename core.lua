...
local interrupts = {
  ["ROGUE"] = {
    ["Kick"] = BS["Kick"];
    ["Kidney Shot"] = BS["Kidney Shot"];
    ["Cheap Shot"] = BS["Cheap Shot"];
  },
  ["WARRIOR"] = {
...
function Interruptor:Announce(sName)
  if not self.db.profile.Active then return end
  ...
  if optChannel == "RAID" and getGroup == 1 then
    ...
    -- prepare the string
  local announceText = self.db.profile.Announce
  -- $spell
  announceText = string.gsub(announceText, "$spell", sName);
  -- $targetName
  announceText = string.gsub(announceText, "$target", UnitName("target"))
  -- $targetMark
  local markIndex = GetRaidTargetIndex("target")
  if markIndex then
    announceText = string.gsub(announceText, "$mark", "( "..target_icons[markIndex].." )")
  else
    announceText = string.gsub(announceText, "$mark", "");
  end
  --
  ...
  if string.lower(whispertarget) ~= string.lower(NONE) and self:inGroup(whispertarget) then
    SendChatMessage(announceText, "WHISPER", nil, whispertarget)
  end
  --if (msg) then
